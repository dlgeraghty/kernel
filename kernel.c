...
inline void cls(unsigned char *v){
  for (int i = 0; i < w*h*2; i++) {
    v[i] = 0x0;
  }
}

inline void prints(char *p, int *x, int *y, unsigned char *v){
  for(int i = 0; p[i] != '\0'; ++i){
    printc(p[i], 0x09, x, y, v);
  }
}

inline int inport(unsigned short port){
  int rv;
  __asm__ __volatile__ ("inb %1, %0" : "=a" (rv) : "dN" (port));
  return rv;
}

inline void outport(unsigned short port, unsigned short data){
  __asm__ __volatile__ ("out %1, %0" : : "dN" (port), "a" (data));
}

inline void printcursor(int x,int y){
  outport(0x3D4, 14);
